...
import main.java.Frames.myFrame;
/**
 * serverside for main frame.
 * @author Saimon
 *
 */
public class Server {
	...
	public void serverwasupdated() {
		for(String key: mainDB.getServer_watch().keySet()){
			String url = key;
			if(mainDB.getServer_watch().get(url).isModified()){ 
				int size = mainDB.getDB().size();
				for (int i = 0; i < size; i++) {
					if(mainDB.getDB().get(i).getFrom().equals(url)){
						mainDB.getDB().remove(i);
						i--;
						size = mainDB.getDB().size();
					}
				}
				ArrayList<WifiSpots> scans=Read_fSQL.Read_Again(mainDB.getServer_watch().get(url));
				mainDB.addToDB(scans);
				updateStack();
				display();
